...
window.onload = function() {
	...
	var c = document.getElementsByClassName("calendar")[0],
		w = document.getElementsByClassName("calWrap")[0],
		sL = document.getElementsByClassName("shadow-left")[0],
		sR = document.getElementsByClassName("shadow-right")[0],
		cSW = (c.scrollWidth - w.offsetWidth) / c.scrollLeft;
	
	if (c.scrollWidth != w.offsetWidth) sR.style.opacity = 1;
...
function calender() {
	...
	for (var w=0; w<(months[current][1] + months[current][2]) / 7; w++) {
		...
		for (var d=0; d<7; d++) {
			...
			else if (d == 0 || d == 6) day.style.backgroundColor = "#00000022";
